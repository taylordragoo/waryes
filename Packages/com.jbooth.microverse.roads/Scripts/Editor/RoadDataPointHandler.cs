...
namespace JBooth.MicroVerseCore
{
    ...
    public static class SplineRoadDataHandles
    {
        ...
        public static Road.SplineChoiceData curChoiceData = null;
        public static int curChoiceIndex = 0;
        static int curSelectedID;
        ...
        public static void DataPointHandlesWithRoad<TSpline>(
            this TSpline spline,
            SplineData<Road.SplineChoiceData> splineData,
            bool useDefaultValueOnAdd = false)
                where TSpline : ISpline
        {
            spline.DataPointHandlesWithRoad(splineData, useDefaultValueOnAdd, 0);
        }
                ...
                where TSpline : ISpline
        {
            ...
            DataPointMoveHandles(id, spline, splineData);
        ...
        static void DataPointAddHandle<TSpline>(
            int controlID,
            TSpline spline,
            SplineData<Road.SplineChoiceData> splineData,
            bool useDefaultValueOnAdd,
            ...
            switch (eventType)
            {
                ...
                case EventType.MouseDown:
                        ...
                        && GUIUtility.hotControl == 0)
                    {
                        ...
                        var sd = new Road.SplineChoiceData();
                        splineData.Add(index, sd);// splineData.AddDataPointWithDefaultValue(index, useDefaultValueOnAdd);
                        curChoiceData = null;// splineData.ElementAt(s_NewDataPointIndex).Value as Road.SplineChoiceData;
                        curChoiceIndex = -1;// s_NewDataPointIndex;
                        for (int idx = 0; idx < splineData.Count; ++idx)
                        {
                            if (splineData[idx].Index == index)
                            {
                                curChoiceData = splineData[idx].Value;// splineData.ElementAt(s_NewDataPointIndex).Value as Road.SplineChoiceData;
                                curChoiceIndex = idx;
                                s_NewDataPointIndex = idx;
                            }
                        }
                        
                        curSelectedID = controlID;
                        GUI.changed = true;
                        changed = true;
                        evt.Use();
                    }
                ...
                case EventType.MouseDrag:
                    if (evt.button == 0 && IsHotControl(splineID))
                    {
                        ...
                        //curChoiceData = splineData.ElementAt(s_NewDataPointIndex).Value as Road.SplineChoiceData;
                        //curSelectedID = controlID;
                        GUI.changed = true;
                        changed = true;
                        evt.Use();
        ...
        static void DataPointMoveHandles<TSpline, TData>(int controlID, TSpline spline, SplineData<TData> splineData)
                where TSpline : ISpline
        {
            ...
            for (int dataIndex = 0; dataIndex < splineData.Count; dataIndex++)
            {
                ...
                if (GUIUtility.hotControl == s_DataPointsIDs[dataIndex])
                {
                    ...
                    //curChoiceData = splineData.ElementAt(newDataIndex).Value as Road.SplineChoiceData;
                    //curSelectedID = controlID;
            ...
            switch (eventType)
            {
                ...
                case EventType.MouseDown:
                        ...
                        && GUIUtility.hotControl == 0)
                    {
                        ...
                        SetClosestSplineData(spline, splineData, controlID);
                        GUI.changed = true;
                        changed = true;
                        evt.Use();
                    }
        ...
        static void DrawSplineDataHandle(int controlID, Vector3 position, float size)
        {
            ...
            if (controlID == curSelectedID) // selected
                handleColor = Color.red;
        ...
        static void SetClosestSplineData<TSpline, TData>(TSpline spline, SplineData<TData> splineData, int controlID) where TSpline : ISpline
        {
            var evt = Event.current;
            var ray = HandleUtility.GUIPointToWorldRay(evt.mousePosition);
            ...
            int index = 0;
            foreach (var s in splineData)
            {
                float kn = SplineUtility.GetNormalizedInterpolation(spline, s.Index, PathIndexUnit.Normalized);
                float dist = math.abs(kn - t);
                if (dist < closest)
                {
                    closest = dist;
                    curChoiceData = s.Value as Road.SplineChoiceData;
                    curChoiceIndex = index;
                    curSelectedID = controlID;
