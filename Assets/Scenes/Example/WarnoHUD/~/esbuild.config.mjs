...
import { copyAssetsPlugin, importTransformPlugin, tailwindPlugin, ussModulesPlugin } from "onejs-unity/esbuild"
...
const config = {
    ...
    // Import plain .uss files as strings, so styles can be embedded in the bundle
    // via compileStyleSheet(). Required for styles to work in builds, where
    // loadStyleSheet() can't reach files from the working directory.
    // (.module.uss files are handled by ussModulesPlugin instead.)
    loader: { ".uss": "text" },
    ...
    plugins: [
        ...
        // Copy SVG icons into OneJS' generated asset manifest.
        copyAssetsPlugin(),
    ],
}
